...
@Component({
  ...
  templateUrl: './library-stock-list.component.html',
  styleUrl: './library-stock-list.component.css'
})
export class LibraryStockListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  crewId = 0;
  pageTitle = 'Offerings';
  stockKind: LibraryOfferingKind = 'Consumable';
  holderLabel = 'From';
  ...
  ngOnInit() {
    ...
    this.libraryService.getCategories({ inUseOnly: true, kind: this.stockKind }).subscribe({
      next: categories => {
  ...
  private loadItems(reset: boolean) {
    ...
    const loader = this.stockKind === 'Service'
      ? this.libraryService.getServicesUnits({
