 ...
 */
export function sniffMediaMime(bytes: ArrayBuffer | Uint8Array | null | undefined): string | null {
  if (!bytes) {
    return null;
  }
...
export function resolveBlobMime(
  ...
  const declaredTrimmed = (declared || '').trim().toLowerCase();
  if (
    declaredTrimmed
  ...
  return sniffMediaMime(bytes) || declaredTrimmed || 'application/octet-stream';
}
