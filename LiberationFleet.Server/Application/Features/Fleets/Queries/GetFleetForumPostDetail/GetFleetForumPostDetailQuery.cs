...
namespace LiberationFleet.Server.Application.Features.Fleets.Queries.GetFleetForumPostDetail;

public record GetFleetForumPostDetailQuery(int PostId) : IRequest<ForumDetailResponse>;

public class GetFleetForumPostDetailQueryHandler(
    ...
    IUserBlockRepository blockRepository) : IRequestHandler<GetFleetForumPostDetailQuery, ForumDetailResponse>
{
    public async Task<ForumDetailResponse> Handle(GetFleetForumPostDetailQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            return new ForumDetailResponse { Success = false, Message = "Not a fleet forum post." };
        }

        var fleetId = post.FleetId.Value;
        if (!await fleetRepository.IsUserInFleetAsync(userId, fleetId, cancellationToken))
        {
            return new ForumDetailResponse { Success = false, Message = "You are not in this fleet." };
        }

        var membership = await membershipRepository.GetActiveMembershipAsync(userId, cancellationToken);
        if (membership is null)
        {
            return new ForumDetailResponse { Success = false, Message = "You are not in a crew." };
        }
